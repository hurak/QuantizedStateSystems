...
Solve the initial value problem (IVP) for a given set of first-order ODEs with inputs using the method of hysteretically quantized state system (QSS) or order 1.

For a set of first-order explicit ODEs (aka state equations) `ẋ = f(x,u)`, with the initial value `x₀` specified at an initial time `t₀`, and possibly a piecewise (control) input `u` given by a pair of vectors `tuarray` and `uarray`of times and values, find the solution on the time span `tspan=(t₀,t₁)`. The only parameter for the method is the quantum `Δq`, while the hysteresis band has the same width as the quantum.
...
Note that for a first-order system with no external events the `x` and `q` arrays are identical up to the initial values (`x₀` before vs. after quantization).
"""
function qss1(f,x₀::Vector{<:Real},tspan,Δq,tuarray,uarray)
    (t₀,t₁) = tspan             # Initial and final times for the simulation.
    if ~isempty(tuarray) && tuarray[1]==t₀  # Setting the initial value of the input:
        u = uarray[1]           # - either to the first value of the (piecewise) input,
        popfirst!(uarray)
    ...
    t = t₀                      # Initializing the current time.
    x = x₀                      # Initializing the current state.
    tarray = [[t₀] for _∈x]     # Initializing an array of arrays of times of unknown length.
    xarray = [[z] for z∈x]      # Initializing an array of arrays of state variables.
    q = [Δq[i]*floor(x[i]/Δq[i]) for i=1:length(x)] # Vector of quantized initial state variables.
    tqarray = [t,]              # Initializing an array of times for the quantized state variable.
    qarray = [q,]               # Initializing an array of (samples of) the quant. state variable.
    while t < t₁
        k = f(q,u,t)            # Derivative corresponding to the quantized state.
